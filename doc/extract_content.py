...
# root directory, while geckOS's documentation files are spread around
# the tree.
#
# This script has been minimally modified from the one used to build
# Zephyr's documentation. All credit goes to their developers.
...
# Represents the content which must be extracted from the geckOS tree,
...
def src_deps(base, src_file, dest, src_root):
    # - base: the geckOS directory containing src_file
    ...
    # dependency's *relative path from base*.
    ...
    dst_dir = path.join(dest, path.relpath(src_dir, start=base))
...
def find_content(base, src, dest, fnfilter, ignore, src_root):
    ...
    outputs = []
    output_dirs = set()
    for dirpath, dirnames, filenames in os.walk(path.join(base, src)):
        ...
        # needs to exist.
        dst_dir = path.join(dest, path.relpath(dirpath, start=base))
        ...
        for src_rel in sources:
            ...
            deps = src_deps(base, src_abs, dest, src_root)
...
def main():
    parser = argparse.ArgumentParser( description='''Recursively copy
            documentation files from geckOS project root directory to a
            destination folder, along with files referenced in those .rst files
            by a configurable list of directives: {}.'''.format(DIRECTIVES))

    parser.add_argument('--outputs',
                        help='If given, save input/output filenames to this path')
    ...
    parser.add_argument('geckOS_project_root',
                        help='''A path pointing to the
                        root level directory of the geckOS project.''')
    ...
    for fnfilter, source, dest in content_config:
        content = find_content(base, source, dest, fnfilter, ignore,
